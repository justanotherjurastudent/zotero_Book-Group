...
describe("Book Group", function () {
	...
	describe("main item tree", function () {
		...
		const rowElement = (item: Zotero.Item) =>
			win.document.getElementById(`${tree.id}-row-${tree.getRowIndexByID(item.id)}`);
		...
		it("groups and decorates rows when sorted by the grouping column", async function () {
			...
			assert.isFalse(rowElement(standalone)?.classList.contains("bookgroup-child") ?? false);
		});
		...
		it("decorates rows again after the window is re-attached", async function () {
			const { trees } = getServices();
			trees.detach(win);
			await waitFor(() => !rowElement(chapterEarly)?.classList.contains("bookgroup-child"));
	...
	describe("column header context menu", function () {
		...
		it("is not added to View → Columns or when disabled", function () {
			...
			Zotero.Prefs.set(prefKey, false, true);
			try {
				assert.isNull(buildPicker().querySelector("#bookgroup-column-picker-sort"));
			} finally {
				Zotero.Prefs.set(prefKey, true, true);
			}
		});
	...
	describe("contribution linked to several volumes", function () {
		...
		it("prefers the volume with the same title and a shared editor", async function () {
			...
			const otherVolume = await createWithEditors("book", { title: "BG Festschrift" }, [
				"Weber",
			]);
			const matchingVolume = await createWithEditors("book", { title: "BG Kommentar" }, [
				"Weber",
				"Klein",
			]);
			...
			await waitFor(() => relations.index.getParentID(section.id) === matchingVolume.id);
		});

		it("falls back to the lowest item ID without a match", async function () {
			const { relations } = getServices();
			const first = await createWithEditors("book", { title: "BG Band A" }, ["Adler"]);
			const second = await createWithEditors("book", { title: "BG Band B" }, ["Berg"]);
			const section = await createWithEditors(
				"bookSection",
			...
			await waitFor(
				() => relations.index.getParentID(section.id) === Math.min(first.id, second.id),
			);
		});
	});
	...
	describe("preference pane", function () {
		...
		it("offers a palette color picker bound to the preference", async function () {
			...
			await waitFor(() => {
				...
				return !!prefsWindow?.document.querySelector("#bookgroup-pref-bg-color button");
			});
			const picker = prefsWindow!.document.getElementById("bookgroup-pref-bg-color") as any;
