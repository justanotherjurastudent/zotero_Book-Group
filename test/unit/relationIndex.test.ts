...
const rel = (subjectID: number, objectKey: string, objectLibraryID = 1): RelationRow => ({
	subjectID,
	objectKey,
	objectLibraryID,
});
...
describe("volume metadata matching", () => {
	const editor = (lastName: string, firstName = "") => creatorKey({ lastName, firstName });
	...
	it("requires both title and editor", () => {
		...
		assert.equal(
			matchesVolume({ volumeTitle: "", editors: [] }, { volumeTitle: "", editors: [] }),
			false,
		);
	});
