...
export class GroupSorter {
  private rootKeys = new Map<number, string[]>();
  ...
  getVisibleParentID(
    ...
    const parentID = this.relations.index.getParentID(itemID);
  ...
  hasVisibleChildren(itemID: number, isVisible: VisibilityCheck): boolean {
    return this.relations.index.getChildIDs(itemID).some(isVisible);
  }
  ...
  getEntry(item: unknown, isVisible: VisibilityCheck): GroupEntry | null {
    ...
    return {
      id: item.id,
      rootID: root.id,
      rootKey: this.getRootKey(root),
      rank: parent ? 1 : 0,
      pageStart: parent
        ? parsePageStart(item.getField("pages") as string)
        : null,
  ...
  compare(
    ...
    const entryA = this.getCachedEntry(a, isVisible, entryCache);
    const entryB = this.getCachedEntry(b, isVisible, entryCache);
    if (!entryA && !entryB) {
      return null;
    }
  ...
  private getRootKey(item: Zotero.Item): string[] {
    let key = this.rootKeys.get(item.id);
    if (!key) {
      key = this.computeRootKey(item);
      this.rootKeys.set(item.id, key);
    }
    return key;
  }
   ...
   */
  private computeRootKey(item: Zotero.Item): string[] {
    ...
    return this.settings.current.baseSort === "title"
      ? [title, creator, year]
      : [creator, year, title];
  }
}
