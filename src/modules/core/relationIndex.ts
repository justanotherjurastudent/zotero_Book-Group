...
export function matchesVolume(
  contribution: VolumeMetadata,
  container: VolumeMetadata,
): boolean {
...
export function findAmbiguousContributions(
  links: Iterable<GroupLink>,
): number[] {
...
export class RelationIndex {
  ...
  private parentToChildren = new Map<number, number[]>();
  ...
  rebuild(links: Iterable<GroupLink>): boolean {
    ...
    let changed = childToParent.size !== this.childToParent.size;
    if (!changed) {
      for (const [childID, parentID] of childToParent) {
        if (this.childToParent.get(childID) !== parentID) {
          changed = true;
          break;
        }
      }
    ...
    for (const id of ids) {
      ...
      if (parentID !== undefined) {
        ...
        if (siblings) {
          ...
          if (remaining.length) {
            this.parentToChildren.set(parentID, remaining);
          } else {
  ...
  isChild(id: number): boolean {
    return this.childToParent.has(id);
  }

  isParent(id: number): boolean {
    return this.parentToChildren.has(id);
  }
